...
def test_non_lin(neuron=10, niter=1000, gradient_step=1e-3, batch_size=None):    
    ...
    datax, x_test, datay, y_test = train_test_split(datax, datay, test_size=0.2, random_state=42)
    ...
    ypred = nonlin.predict(x_test)
    y_test = np.argmax(y_test, axis=1)
    
    print("\nTemps d'exécution: ", time.time() - time_start )
    print("Score de bonne classification: ", np.mean( ypred == y_test ))
    plot(x_test, ypred, nonlin, name='Regression non Linéaire, n_neurons = {}, niter = {}, gradient_step = {}'.format(neuron, niter, gradient_step))
